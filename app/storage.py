...
TABLE_DEFINITIONS: dict[str, str] = {
    ...
    "approvals": "id TEXT PRIMARY KEY, run_id TEXT, proposal_id TEXT, sender_id TEXT, raw_message TEXT, parsed_action TEXT, authorized INTEGER, status TEXT, created_at TEXT, consumed_at TEXT, reply_to_message_id TEXT, proposal_targeting_method TEXT, acknowledgement_status TEXT, approval_received_at TEXT, acknowledgement_sent_at TEXT, acknowledgement_delay_seconds REAL, final_revalidation_started_at TEXT, final_revalidation_completed_at TEXT, price_refreshed_at TEXT, refreshed_price REAL, refreshed_price_age_seconds REAL, price_move_bps_since_proposal REAL, final_order_decision TEXT, final_block_reason TEXT, authority_envelope_json TEXT, authority_fingerprint TEXT, UNIQUE(proposal_id, status) ON CONFLICT ABORT",
...
RUNTIME_ADDITIVE_COLUMNS: dict[str, dict[str, str]] = {
    ...
    "order_intents": {
        "approval_authority_fingerprint": "TEXT",
        "strategy_registry_snapshot_id": "TEXT", "strategy_sleeve": "TEXT", "sleeve_allocation_id": "TEXT",
        "sleeve_notional_ceiling": "REAL", "sleeve_stop_risk_ceiling": "REAL",
        "winner_expansion_decision_id": "TEXT", "pyramiding_milestone_id": "TEXT",
        "pyramiding_milestone_key": "TEXT", "management_mode": "TEXT",
...
class Storage:
    ...
    def recover_stale_paper_runs(self, current_run_id: str) -> list[str]:
        """Close scanner runs left running by a process restart.

        The scanner lock is acquired before this method is called, so any
        older paper run still marked ``running`` no longer has scanner
        authority.  Keep the recovery explicit and auditable without touching
        orders, intents, fills, or any other trading ledger state.
        """
        now = iso_now()
        recovered: list[str] = []
        with self.connect() as conn:
            rows = conn.execute(
                """SELECT id, started_at FROM runs
                   WHERE mode='paper' AND status='running' AND id<>?
                   ORDER BY started_at""",
                (current_run_id,),
            ).fetchall()
            for row in rows:
                detail = json_dumps(
                    {
                        "reason": "scanner_restart_after_previous_process_lost_authority",
                ...
                updated = conn.execute(
                    """UPDATE runs
                       ...
                       WHERE id=? AND mode='paper' AND status='running'""",
                    (now, detail, row["id"]),
                )
                ...
                conn.execute(
                    """INSERT INTO audit_events(run_id,event_type,actor,detail,created_at)
                       VALUES(?,?,?,?,?)""",
                    (row["id"], "paper_run_recovered_after_restart", "system", detail, now),
                )
                conn.execute(
                    """INSERT INTO audit_events(run_id,event_type,actor,detail,created_at)
                       VALUES(?,?,?,?,?)""",
