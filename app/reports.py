...
SHEETS: list[tuple[str, str | None]] = [
    ...
    ("Crypto Strategy Decisions", "SELECT id,run_id,research_run_id,symbol,selected_strategy,action,lifecycle,signal_eligible,proposal_authorized,execution_authorized,stop_price,target_price,expected_reward_r,blockers_json,market_evidence_id,config_hash,formula_version,schema_version,as_of,created_at,decision_fingerprint FROM crypto_strategy_decisions ORDER BY created_at DESC,symbol"),
    ("Crypto Proposal Previews", "SELECT id,run_id,symbol,strategy,action,request_basis,status,manual_approval_eligible,execution_authorized,strategy_decision_id,risk_decision_id,risk_snapshot_id,sizing_decision_id,capability_snapshot_id,market_evidence_id,created_at,expires_at,display_json,display_fingerprint,proposal_fingerprint,config_hash,formula_version,schema_version FROM crypto_proposal_previews ORDER BY created_at DESC,symbol"),
    ("Cross Asset Allocation", "SELECT id,run_id,as_of,expires_at,portfolio_snapshot_id,candidate_set_fingerprint,policy_fingerprint,execution_authorized,config_hash,formula_version,schema_version,plan_json,plan_fingerprint FROM cross_asset_allocation_plans ORDER BY as_of DESC"),
