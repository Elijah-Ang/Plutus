...
SHEETS: list[tuple[str, str | None]] = [
    ...
    ("Capacity-Limited Candidates", "SELECT ps.symbol, ps.tier, ps.setup_type, ps.score, ps.not_proposed_reason, pb.blocker, pb.reason, ps.created_at FROM performance_setups ps JOIN performance_blockers pb ON pb.setup_id=ps.id WHERE pb.blocker IN ('pending_proposal_limit','max_daily_trades','cooldown') OR lower(pb.reason) LIKE '%simultaneous candidate%' OR lower(pb.reason) LIKE '%pending%' OR lower(pb.reason) LIKE '%cooldown%' ORDER BY ps.created_at DESC"),
