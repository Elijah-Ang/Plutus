...
from app.phase3_risk import PHASE3_SCHEMA_VERSION, PROFILE_VERSION, Phase3Controller  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Explicit paper-only Phase 3 activation gate"
    )
    ...
    if (
        not args.allow_phase3_activation
        or os.getenv("TRADINGAGENT_ALLOW_PHASE3_ACTIVATION") != "YES_ACTIVE_PAPER_ONLY"
    ):
    ...
    if (
        ...
        or manifest.get("schema_version") != PHASE3_SCHEMA_VERSION
    ):
        raise SystemExit("release is not a Phase 3 paper release")
    config = load_config()
    if (
        not config.get("phase3", {}).get("active")
    ...
    storage.execute(
        "INSERT INTO phase3_activation_events VALUES(?,?,?,?,?,?,?,?,?)",
        (
            str(uuid.uuid4()),
