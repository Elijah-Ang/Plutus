...
- **Dynamic Paper Sizing**: BUY/ADD proposals use one versioned notional policy. The moderate-paper defaults are `$250` initial and `$100` add, then the final notional is the minimum of every applicable stage, validated-stop-risk, equity, cash, buying-power, symbol, cluster, portfolio, allocation, exploration, and optional absolute ceiling. No ceiling is raised to satisfy a minimum; constrained entries block.
