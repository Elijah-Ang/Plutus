...
- **Staged Crypto Lane**: BTC/USD and ETH/USD stay `research_only` by default. Each research cycle binds current Alpaca Assets API precision/tradability and current quote/order-book evidence to a verified paper account. Separate Decimal sizing and portfolio-risk records now constrain quantity/notional precision, fees, stop loss, cash, buying power, positions, pending exposure, sleeve/symbol/cluster exposure, heat, loss, drawdown, and volatility, but schema and config still make them incapable of authorizing execution. The generic equity adapter rejects crypto before broker I/O. `paper_watch` can record hypothetical candidates with no proposals or orders. Enabling a later proposal stage requires a separate reviewed change. See [CRYPTO_DATA_AND_BROKER_CAPABILITY.md](docs/CRYPTO_DATA_AND_BROKER_CAPABILITY.md) and [CRYPTO_SIZING_AND_RISK.md](docs/CRYPTO_SIZING_AND_RISK.md).
