...
- **Staged Crypto Lane**: BTC/USD and ETH/USD stay `research_only` by default. `paper_watch` can record hypothetical candidates with stop/target/risk metadata but no proposals or orders; `paper_proposal` is runtime-evidence-gated, Alpaca-paper-only, manual-approval-only, and keeps EODHD as research context rather than a final trading source.
