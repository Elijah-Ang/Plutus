...
TradingAgent is a local, supervised trading assistant designed for education and Alpaca paper trading. It checks Mac power, connectivity, configuration, market state, deterministic risk limits, proposal expiry, durable paper authority, and final order state. SQLite stores the audit trail and Excel exports provide a reviewable report.

It does **not** provide financial advice, let an AI decide or place orders, enable live trading, move cash, automate account login, or retry an uncertain order. The configured paper authority is deterministic and bounded: ordinary paper entries/exits may be submitted autonomously after durable reservation and final revalidation, while `manual_only` remains available as a compatibility mode. Telegram is notification/control infrastructure, not an AI decision-maker.
...
- **Continuous Crypto Lane**: BTC/USD and ETH/USD use the separate Alpaca spot-paper lane. Each cycle binds current Assets API precision/tradability and quote/order-book evidence to a verified paper account. Decimal sizing, portfolio risk, durable reservations, and broker reconciliation constrain quantity/notional precision, fees, stop loss, cash, buying power, positions, pending exposure, sleeve/symbol/cluster exposure, heat, loss, drawdown, and volatility. The generic equity adapter rejects crypto before broker I/O, and `cancel_pending` reservations remain active until reconciliation proves the terminal state. The lane is independent of EODHD and can trade continuously while equities are closed. See [CRYPTO_DATA_AND_BROKER_CAPABILITY.md](docs/CRYPTO_DATA_AND_BROKER_CAPABILITY.md), [CRYPTO_SIZING_AND_RISK.md](docs/CRYPTO_SIZING_AND_RISK.md), and [CRYPTO_SUPERVISED_PAPER_LANE.md](docs/CRYPTO_SUPERVISED_PAPER_LANE.md).
...
- **Research Candidate Semantics**: An equity research candidate has passed the pre-market universe scan, filtering, EODHD/local quantitative scoring, and deterministic candidate brief generation. It is worth tracking and explaining, but it is not tradable and cannot create trade proposals or orders.
...
- Bounded paper auto-execution is supported only in `mode: paper` with `auto_execution_mode: autonomous_paper`; live trading remains impossible. `manual_only` requires an authorized Telegram approval for ordinary paper orders.
...
- Missing EODHD keys, no internet, provider downtime, and low battery skip Dynamic Universe research safely. Equity research preflight failures block Dynamic Universe research, but they do not block the independent Alpaca crypto research/execution lane; existing static scanning can continue when normal scanner preflight passes.
