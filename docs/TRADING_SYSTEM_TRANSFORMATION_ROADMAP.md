...
| P0-E-002 | Approval workflow links accepted approval to intent | Recover accepted work | P0-B/E-001 | Workflow has update uniqueness, intent link, version, terminal/manual review | implemented | 8d7d1f5 | schema/integrity tests | P0-R | P0-B | Atomic consume+intent creation remains partial for pre-intent final-validation failures |
| P0-E-003 | Startup/periodic recovery sweep | Restart safety | P0-E-002 | Detects received updates, approvals without intents, awaiting/unknown/stale intents, terminal reservations | implemented | 8d7d1f5 | recovery heartbeat queries | P0-R | P0-B | Reserved intents are surfaced, not auto-submitted, pending explicit recovery worker proof |
| P0-E-004 | Authorization, targeting, expiry, plain-yes, paper yes-all unchanged/stricter | Preserve approval safety | P0-E-001 | Existing routing tests pass | partial | 8d7d1f5 | full suite pending | P0-R | P0-B | Await full regression result |
...
| P0-H-003 | Bounded not-found policy and classified attempt telemetry | One 404 is not proof | P0-H-002 | Three observations before confirmed absence/release | implemented | 8d7d1f5 | targeted multi-404 test still needed | P0-R | P0-B | Threshold may be made validated config after evidence |
| P0-H-004 | Found/not-yet-found/absent/failure/divergence/manual-review telemetry | Operational signal quality | P0-H-001 | Reconciliation result and attempt rows separate outcomes | implemented | 8d7d1f5 | targeted reconciliation suite | P0-R | P0-B | None |
...
| P0-I-004 | Canonical snapshot consistency across batch/final/display | One source of truth | P0-I-001 | Same snapshot ID/inputs produce checks and labels | partial | 8d7d1f5 | focused consistency test pending | P0-R | P0-B | Existing report paths still need full conversion |
...
| P0-N-003 | All 20 required crash boundaries | Prove recovery | P0-B–K | Each named boundary deterministic and passing | partial | 8d7d1f5 | covered: intent boundary, ambiguity, duplicate fill, reopen, never-submitted; remainder pending | P0-R | P0-B | Full matrix still required before Phase 0 completion |
| P0-N-004 | Required invariants and deterministic concurrency | Race safety | P0-B–H | duplicate worker produces one intent/submit; SQLite contention covered | partial | 8d7d1f5 | two-worker test; broader lock contention pending | P0-R | P0-B | Approval/reservation/reconcile race matrix incomplete |
| P0-N-005 | Fresh/current/repeat/interrupted migration tests preserving all named data | Safe 543 MB evolution | P0-B schema | Temp fixtures and fingerprints; no production DB | partial | 8d7d1f5 | fresh/repeat pass; current-schema/interruption fingerprints pending | P0-R | P0-B | Exact production-schema fixture must be generated without opening production DB for write |
...
| P0-LOG-001 | Redacted transition/recovery logging and noise reduction | Privacy/operability | P0-C/E/H | Internal IDs only; no raw Telegram/account identifiers; local-only rows excluded | partial | 8d7d1f5 | code review; secret scan pending | P0-R | P0-B | Existing legacy audits still contain sender fields and need separate migration |
