...
- Crypto risk controls are explicit: no live trading, no crypto paper proposals by default, no margin, no shorting, long-only spot, BTC/USD and ETH/USD only for the first staged rollout, add-to-winner disabled, limit orders only for v1, $5 maximum hypothetical notional, 1% maximum crypto exposure setting, and fresh Alpaca price/spread requirements for any future proposal path.
