...
- [crypto_research.py](../app/crypto_research.py): Deterministic staged crypto lane for BTC/USD and ETH/USD by default. Stage 1 `research_only` records price, return, volatility, trend, liquidity/spread, score, risk, Performance Lab shadow events, and counterfactual placeholders without admitting crypto to the equity scanner or creating proposals/orders. Stage 2 `paper_watch` records hypothetical proposal candidates only. Stage 3 `paper_proposal` is runtime-evidence-gated and can create manual-approval paper proposals only when crypto config gates are explicitly enabled.
...
- Stage 2 `crypto.mode=paper_watch` keeps `paper_trading_enabled=false` and `proposals_enabled=false`. It records rows in `crypto_paper_watch_candidates` with entry price, stop price, stop distance %, take-profit target, risk/reward ratio, spread bps, volatility regime, position size, max loss estimate, provider coverage, and Alpaca final price timestamp, but it creates no `trade_proposals`, Telegram approval prompts, approvals, or broker orders.
- Stage 3 `crypto.mode=paper_proposal` requires `paper_trading_enabled=true`, `proposals_enabled=true`, Alpaca paper-only operation, limit-order configuration, and a runtime evidence gate before a pending crypto proposal can be recorded. The evidence gate requires multiple BTC/USD and ETH/USD scheduler cycles with fresh Alpaca data, spread coverage or explicit blockers, Performance Lab rows, quiet-hours DB recording, no unexpected proposals/orders, and no unresolved provider errors.
