...
- [crypto_research.py](../app/crypto_research.py): Deterministic staged crypto lane for BTC/USD and ETH/USD by default. Stage 1 `research_only` records price, return, volatility, trend, liquidity/spread, score, risk, Performance Lab shadow events, and counterfactual placeholders without admitting crypto to the equity scanner or creating proposals/orders. Stage 2 `paper_watch` records hypothetical proposal candidates only. Stage 3 is readiness-report-only in this implementation; it cannot create crypto proposals automatically, even if the evidence gate passes.
...
- Crypto has a separate staged lane outside the equity scanner. Default symbols are BTC/USD and ETH/USD; SOL/USD is optional but not enabled unless configured. Crypto lanes are `crypto_raw`, `crypto_research_candidate`, `crypto_observation`, `crypto_paper_watch`, `crypto_paper_tradable`, and `crypto_trade_proposal`. The default config is Stage 1: `crypto.mode=research_only`, `crypto.paper_trading_enabled=false`, and `crypto.proposals_enabled=false`.
...
- Stage 3 must not be enabled automatically in this implementation. Even if the runtime evidence gate passes, crypto records only a Stage 3 readiness report in `crypto_paper_watch_candidates`; enabling real Stage 3 paper proposals requires a separate explicit user-approved config-change task and a separate commit.
...
- The versioned crypto strategy evaluator separately records time-series trend, breakout continuation, pullback-in-trend, and volatility-adjusted momentum decisions from the exact point-in-time hourly bar set. It persists `RESEARCH_ONLY`, `proposal_authorized=false`, and `execution_authorized=false`, and independently recomputes on load.
- Crypto proposal previews bind an exact strategy decision to verified capability, market, paper-account, portfolio-risk, and Decimal sizing evidence. They show the complete future manual approval surface, but are constrained to `research_only_preview`, are not sent to Telegram, and cannot create ordinary proposals, approvals, intents, reservations, or broker calls.
...
- Crypto RiskEngine policy is explicit: do not blanket-unblock `asset_class=crypto`. Stage 1 and Stage 2 crypto remain execution-blocked by design. A future crypto-specific allow path may exist only for Stage 3 `paper_proposal` mode after explicit enablement; it must preserve final validation, paper-only mode, live-disabled state, no auto-execution, Alpaca paper authority, and all crypto-specific spread/position/open-order checks.
- Crypto risk controls are explicit: no live trading, no crypto paper proposals by default, no margin, no shorting, long-only spot, BTC/USD and ETH/USD only for the first staged rollout, add-to-winner disabled, a $1 minimum and $5 maximum initial paper order, at most two positions, 1% crypto gross exposure, 0.60% per symbol, 1% `crypto_major` cluster exposure, 0.01% per-trade stop risk, 0.05% crypto heat, 20% cash reserve, account/crypto loss halts, 6% drawdown halt, volatility throttle/halt, and fresh Alpaca price/spread requirements. All percentages are percentage points of current paper equity.
