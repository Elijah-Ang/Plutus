...
## 1. Project Purpose
TradingAgent is a supervised, paper trading assistant that scans a watchlist of symbols, evaluates technical rules, generates trade proposals, conducts AI-assisted risk and caution reviews, and executes only bounded Alpaca paper orders. The configured `autonomous_paper` authority can submit ordinary paper entries/exits after durable reservation and final revalidation; `manual_only` remains an explicit compatibility mode.
- The bot can scan, propose, reserve, and execute ordinary paper orders only through the selected bounded paper authority. Autonomous authority is deterministic and cannot be enabled for live trading.
...
- The current build has non-configurable code capability gates: live trading remains unsupported, while bounded autonomous paper entries/exits are supported only through the durable authority path.
...
3. **Safety/Control**: Local database records, `RiskEngine` limits, durable reservations, final revalidation, and either manual Telegram approval or deterministic autonomous paper authority.
...
- [crypto_research.py](../app/crypto_research.py): Deterministic continuous crypto lane for BTC/USD and ETH/USD by default. It records price, return, volatility, trend, liquidity/spread, score, risk, Performance Lab evidence, and point-in-time strategy decisions without admitting crypto to the equity scanner. Eligible entries flow through the isolated crypto paper ledger and the selected manual/autonomous authority; they never use the equity adapter.
- [crypto_capabilities.py](../app/crypto_capabilities.py): Versioned, immutable Alpaca spot-crypto protocol and current Assets API authority. It binds verified paper-account identity, BTC/USD and ETH/USD tradability, precision/minimums, continuous-market/feed semantics, config/formula identities, expiry, relational evidence and fingerprints; this module has no order-construction or submission capability.
- [crypto_market_data.py](../app/crypto_market_data.py): Per-symbol immutable US-feed quote, latest-trade and order-book evidence with timestamps, sizes, spread/depth derivation, freshness, capability/config binding and a recomputable fingerprint. Missing/stale/crossed structural evidence fails authority; wide-spread or shallow-book evidence is retained but marked execution-ineligible.
- [crypto_sizing.py](../app/crypto_sizing.py): Separate canonical Decimal crypto sizing. It binds the exact capability, market and portfolio-risk snapshots; applies current Assets API minimums/increments, side-aware price rounding, cents/nine-decimal request precision, conservative entry/exit fees and adverse stop slippage; and never rounds quantity, notional or stop risk above authority.
- [crypto_risk.py](../app/crypto_risk.py): Immutable paper account, position, open-order, hourly-volatility, account/crypto-loss and durable intent/reservation evidence. It de-duplicates the same broker order/reservation by client identity, recomputes total/sleeve/symbol/cluster/cash/buying-power/heat capacity in a coherent SQLite transaction, and emits research-only risk decisions that are schema-constrained from authorizing execution.
...
- [broker_alpaca.py](../app/broker_alpaca.py): Integrates Alpaca Paper, exposes read-only clock/loss/order lookup methods plus explicit crypto Assets/bars/quote/trade/order-book reads on the Alpaca US feed, rejects crypto at the equity `DAY`-order adapter before I/O, and rejects all live clients in this build.
...
- [capabilities.py](../app/capabilities.py): Non-configurable live-trading gate plus capability constants for bounded autonomous paper entries/exits and protective paper exits.
...
- `paper_tradable`: eligible for scanner/proposal evaluation only after deterministic promotion, observation evidence, Alpaca-compatible US lane classification, Alpaca paper tradability confirmation, clean risk classification, and recorded evidence. It is not a guaranteed proposal. Static/core paper-tradable symbols come from the configured Alpaca profile; dynamic paper-tradable symbols are promotions from Dynamic Universe research.
  - `demoted`: historical/retired symbols retained for audit.
- After promotion, dynamic paper-tradable symbols use the same Alpaca scanner/proposal mechanism as static symbols. EODHD can discover, research, score, support observation review, promotion/demotion context, candidate briefs, and reporting, but it is not a required proposal input for already-promoted dynamic paper-tradable symbols.
- Alpaca is the trading source of truth for active/tradable asset status, proposal prices, scanner bars/history, proposal freshness, positions, open orders, buying power/account state, final validation, and order placement. The equity real-time feed is explicitly configured as `iex` or `sip` and passed to latest-trade and latest-quote requests. Feed identity plus the exact bid, ask, timestamp, spread and bounded limit are immutable display authority. Missing, stale, crossed, wrong-feed, or over-limit quotes block before proposal display and again at approval. Stale/missing EODHD data, EODHD cooldown, or EODHD unavailability alone must not block an already-promoted dynamic paper-tradable symbol when Alpaca trading data is fresh.
...
- Crypto has a separate continuous lane outside the equity scanner. Default symbols are BTC/USD and ETH/USD; SOL/USD is optional but not enabled unless configured. Crypto research and execution use Alpaca evidence and the isolated `crypto_paper_*` ledger; they do not depend on EODHD or the equity market clock. The configured supervised paper lane uses the selected manual/autonomous paper authority and remains live-disabled, long-only, cash-funded, and reservation/reconciliation bound.
- Stage 2 `crypto.mode=paper_watch` keeps `paper_trading_enabled=false` and `proposals_enabled=false`. It records rows in `crypto_paper_watch_candidates` with entry price, stop price, stop distance %, take-profit target, risk/reward ratio, spread bps, volatility regime, position size, max loss estimate, provider coverage, and Alpaca final price timestamp, but it creates no `trade_proposals`, `proposal_batches`, Telegram approval prompts, approvals, orders, or fills.
- Crypto entry proposals are not ordinary equity proposals. They are created only after current Alpaca capability, market, risk, sizing, strategy, and profitability evidence is bound; autonomous paper authority may then create a deterministic system approval and execute through the crypto ledger after final revalidation.
- Stage 3 `crypto.mode=paper_proposal` may only be enabled by explicit config after the evidence gate passes, BTC/USD and ETH/USD have at least 3 natural scheduler cycles, spread is available or hard-block behavior is verified, no unexpected proposals/orders occurred, and the user approves Stage 3 enablement. The evidence gate requires multiple BTC/USD and ETH/USD scheduler cycles with fresh Alpaca data, spread coverage or explicit blockers, Performance Lab rows, quiet-hours DB recording, no unexpected proposals/orders, and no unresolved provider errors.
- Crypto research and paper execution run 24/7 and do not require the US equity market to be open. Their schedule and digest intervals are separate from equity scans, with SGT quiet hours. During quiet hours, research data can still be recorded, but non-urgent Telegram crypto status is suppressed.
- Crypto data uses a separate authenticated Alpaca crypto client with the US feed selected explicitly. Every cycle records current Assets API and stable hashed paper-account evidence; unavailable, inactive, non-tradable, wrong-class, wrong-exchange, malformed-precision, stale, or expired evidence yields `crypto_capability_unverified`. If the SDK/provider path is unavailable, crypto records missing/provider-unavailable evidence and creates no proposals. EODHD is research/context only and must not be used as a final trading price, tradability, position, open-order, execution, or approval-time revalidation source.
- Crypto scoring is separate from equity thresholds and considers freshness, trend, momentum, realized volatility, ATR-like volatility, drawdown risk, liquidity, and spread. Scores alone cannot create orders; only the complete isolated crypto authority chain can reach paper submission.
- The versioned crypto strategy evaluator separately records time-series trend, breakout continuation, pullback-in-trend, and volatility-adjusted momentum decisions from the exact point-in-time hourly bar set. It independently recomputes on load; downstream proposal and execution authority is granted only by the isolated crypto paper lane.
- Crypto proposal previews bind an exact strategy decision to verified capability, market, paper-account, portfolio-risk, and Decimal sizing evidence. Manual previews remain non-approvable; executable crypto entries use the separate `CryptoPaperLaneStore` proposal/intent/reservation path and never create equity `trade_proposals`.
- Crypto proposal rules are explicit: Stage 3 v1 proposals must not be sent during quiet hours; quiet hours may record research and paper-watch candidates only. Stage 3 must hard-block if Alpaca bid/ask spread is missing or stale. Future Stage 3 final validation must refresh Alpaca crypto bid/ask, spread bps, latest price, position state, open-order state, and config safety before submission.
- Crypto RiskEngine policy is explicit: do not blanket-unblock `asset_class=crypto`. Crypto-specific execution is allowed only through the supervised paper lane after explicit capability, risk, sizing, and final-validation checks; it preserves paper-only mode, live-disabled state, Alpaca paper authority, and all crypto-specific spread/position/open-order checks.
- Crypto risk controls are explicit: no live trading, no margin, no shorting, long-only spot, BTC/USD and ETH/USD only for the first rollout, add-to-winner remains bounded, at most two positions, 5% crypto gross exposure in the cross-asset sleeve, 3% per symbol in the crypto risk policy, 5% `crypto_major` cluster exposure, 0.05% per-trade stop risk, 0.20% crypto heat, 20% cash reserve, account/crypto loss halts, 6% drawdown halt, volatility throttle/halt, and fresh Alpaca price/spread requirements. All percentages are percentage points of current paper equity.
- Crypto blocker labels are explicit and should not collapse to `other`: `crypto_research_only`, `crypto_paper_disabled`, `crypto_proposals_disabled`, `crypto_pair_unsupported`, `crypto_capability_unverified`, `crypto_market_data_unverified`, `crypto_price_stale`, `crypto_spread_too_wide`, `crypto_orderbook_missing`, `crypto_volatility_extreme`, `crypto_liquidity_insufficient`, `crypto_risk_reward_too_low`, `crypto_stop_distance_invalid`, `crypto_quiet_hours_notification_suppressed`, `crypto_existing_position_conflict`, `crypto_pending_order_conflict`, `crypto_pending_proposal_conflict`, `crypto_provider_unavailable`, and `crypto_alpaca_final_price_unavailable`.
...
- The default pre-market research target is configured for 20:30 SGT. When Dynamic Universe research completes, the compact research-status notification is evaluated from the research result, current market phase, snapshot, and suppression rules. This evaluation is independent of whether the later trading preflight blocks proposal/order work on AC power, market-open, or broker trading-only checks.
- A market-closed run with due research is recorded as `research_completed_trading_blocked_market_closed` when research succeeds and trading remains blocked. Mixed trading failures such as `power; market_open` still finish the scanner run as blocked, but do not imply research failed.
...
- Missing EODHD keys record `provider_unavailable: missing_api_key`. Existing static scanner behavior continues, Dynamic Universe research makes no provider calls and creates no new promotions, and the independent Alpaca crypto lane remains eligible to run. After a later successful provider-backed research completion, stale missing-key schedule state is cleared and superseded with `provider_missing_key_state_recovered` audit evidence.
...
- Equity proposal generation remains strict after fallback promotion. A promoted symbol still cannot create an ordinary paper order unless the market is open, fresh execution market data is available, an ENTRY setup passes, cluster/exposure and RiskEngine checks pass, final validation passes, and no stale-data/open-order blocker exists. Manual authority additionally requires Telegram approval; autonomous paper authority binds a durable system authorization instead.
- Research score is the Dynamic Universe quality/ranking score used for discovery, observation, and paper-tradable promotion review. Trade score is the scanner score used for current proposal consideration on paper-tradable symbols. A high Research score cannot create a proposal, and a high Trade score is still insufficient without an ENTRY setup for new buys or a valid ADD setup for held winners, fresh market validation, sizing, exposure/cluster clearance, RiskEngine pass, selected paper authority, and final validation.
- An ENTRY signal is the current rule-based buy setup from scanner bars and trend/volatility checks. Held symbols require separate ADD eligibility: a fresh buy setup plus add-to-winner checks such as profitability, no averaging down, add limits, and risk/exposure clearance. If those checks fail, the digest should describe the ADD blocker rather than implying the symbol was rejected only because no proposal exists.
...
- **Risk-Budgeted Ranked BUY/ADD Proposals**:
  ...
  - Proposal count is uncapped in both ranked-batch and legacy presentation modes. `execution_limits.max_new_buy_proposals_per_cycle`, `max_pending_buy_proposals`, and `max_new_buy_orders_per_day` are `null`; legacy proposal-count fields are inactive. Trade execution, position, exposure, open-risk, freshness, dedupe, approval, and final-validation controls remain authoritative.
  ...
  - Proposal frequency is calibrated from Performance Lab evidence before config changes. Capacity blockers such as pending proposal limits, cooldown, and daily caps are reported separately from true setup-quality blockers such as no ENTRY/ADD signal, stale price, low score, observation-only, research-only, exposure, cluster, and RiskEngine blocks.
  - Increasing proposal capacity, if later justified, does not weaken final validation. Telegram approval, fresh Alpaca price checks, RiskEngine, exposure caps, cluster caps, no-averaging-down rules, duplicate-order checks, approval expiry, market-open gates, and stale-listener approval checks still apply.
...
- **Position Management Engine**:
  ...
  - TAKE_PROFIT / PROFIT_PROTECT / TRAILING_STOP / TIME_STOP proposals are normal paper sell proposals and require the selected paper authority plus final revalidation. They reduce exposure and are not blocked by BUY exposure caps.
  ...
  - GPT may explain or caution on position-management proposals but cannot approve, execute, override risk, or bypass final revalidation. Manual mode still requires Telegram approval.
- **GPT Reviews and Timeout Explanations**:
  ...
  - Normal exit proposals may bypass GPT delays, but never bypass unused-authority consumption, RiskEngine/final validation, price freshness checks, or paper-only/live-disabled gates. Manual mode requires Telegram approval; autonomous paper mode binds durable system authority. Emergency exits remain the guarded exception and can auto-submit paper sells only after final validation passes.
...
- **Expiry Gates**: Blocks execution if the proposal has expired.
  - **Live Trading & Paper Authority**: Live trading is unsupported by code-level capability constants and disabled in configuration. Ordinary paper execution supports only `manual_only` or `autonomous_paper`; YAML and Telegram cannot enable live trading. Manual operation requires Telegram approval; autonomous operation requires deterministic durable authorization and final revalidation.
...
- `crypto_paper_watch_candidates`: Stage 2/3 crypto candidate ledger. It stores hypothetical or evidence-gated proposal metadata, explicit blockers, stop/target/risk/reward fields, provider coverage, and any linked Stage 3 proposal ID.
- `crypto_capability_snapshots`: Immutable per-cycle Alpaca paper spot-crypto contract, account/config/version authority, expiry, failure reasons, evidence and fingerprints.
- `crypto_asset_capabilities`: Snapshot-bound current Assets API identity, flags, minimum quantity, quantity increment and price increment for BTC/USD and ETH/USD.
- `crypto_market_data_evidence`: Capability-bound quote/trade/order-book fields, timestamps/ages, Decimal spread/depth, eligibility, reasons, config/formula identities and evidence fingerprint.
- `crypto_risk_snapshots`: Exact paper account, position, open-order, durable intent/reservation, loss, volatility, aggregate exposure, derived capacity, expiry, config/formula identity and source-family fingerprints.
- `crypto_sizing_decisions`: Exact request basis, side-aware limit/stop prices, current minimums/increments, canonical Decimal quantity/notional/stop risk, fees, slippage, blockers, binding caps and risk/capability/market identities. `execution_authorized` is always zero.
- `crypto_risk_decisions`: Exact risk/sizing relationship, named checks, failure reasons and research-only eligibility. `execution_authorized` is always zero.
...
- **Crypto Capability**: Per-cycle paper account, protocol/config identity, expiry, authority and fingerprint evidence.
- **Crypto Pair Precision**: Current pair-specific Assets API tradability, flags, minimums, increments and fingerprints.
- **Crypto Market Evidence**: Current quote, trade and order-book timestamps/sizes, spread/depth, authority, eligibility and fingerprints.
- **Crypto Portfolio Risk Evidence**: Current paper account, positions, orders, durable exposure, loss, volatility, derived capacities, expiry and fingerprints.
- **Crypto Decimal Sizing**: Current pair precision, canonical quantity/notional/stop risk, estimated fees/slippage, blockers and binding caps.
- **Crypto Risk Decisions**: Named portfolio checks, eligibility and explicit zero execution authority.
- **Crypto Strategy Decisions**: Point-in-time four-strategy metrics, selected setup, stop/target, blockers, lifecycle and explicit zero proposal/execution authority.
- **Crypto Proposal Previews**: Immutable future approval surfaces bound to strategy/risk/sizing evidence and explicitly disabled for approval and execution.
...
- **External position reconciliation**: Supported spot-crypto and equity positions placed outside Plutus are absorbed into an explicit broker-reconciled lifecycle and prospective FIFO lot. Missing protective-risk history is recorded as reconstructed/unavailable, and cross-asset allocation reserves full market value as conservative downside under `external_unmanaged`. Risk-reducing exits remain eligible; risk-increasing adds require verified entry and stop provenance.
...
- **Paper-only scope**: The upgrade changes proposal sizing, measurement, ranking, authority, and reporting only. Live trading remains unsupported and disabled; paper autonomy is bounded to the durable paper authority path.
...
- **Proposal bottleneck reporting**: Performance Lab and reports classify suppressed setup blockers so operator review can distinguish low proposal frequency caused by true setup scarcity from capacity constraints. Report tabs include Proposal Capacity Status, Proposal Bottleneck Summary, Suppressed Setup Blockers, Capacity-Limited Candidates, and Proposal Frequency Audit.
...
- **Lifecycle-accurate populations**: A generated proposal is `proposal_unfilled`, not an actual trade. Approval, blocking, rejection, expiry, supersession, submission-without-fill, ambiguous submission, and cancelled submission remain distinct nonactual evidence classes. Only a durable fill promotes the Performance Lab outcome to `actual_fill`; summary actual-trade counts are recomputed from that fill-bound class.
...
- **Setup tracking line**: Proposal count is uncapped. The digest reports suppressed or observation-only setup count and the top recent blocker without inventing used/remaining proposal capacity. Signal, freshness, dedupe, RiskEngine, exposure, sizing, approval, and final execution gates remain unchanged.
...
- **Proposal blocker wording**: For paper-tradable rows, the digest maps stored scanner state into the most specific display reason available, such as `score below threshold`, `no ENTRY signal`, `already held; no valid add setup`, `stale market data`, `failed freshness validation`, `failed risk sizing`, `cluster exposure limit`, `portfolio exposure limit`, `provider data unavailable`, `Alpaca trading-data/profile block: missing approved scanner profile`, or `proposal builder returned no candidate`. It should not show circular wording such as `Proposal blocked: no proposal` or `Status: Watch — no proposal`.
...
- **Market-closed Dynamic Universe throttling**: Weekend, holiday, post-market, pre-market, catch-up, and other market-closed Dynamic Universe status messages are compared against the last sent stable snapshot in `audit_events`. The snapshot includes both counts and symbol sets for research candidates, observation symbols, Alpaca-compatible observation symbols, global research-only symbols, dynamic paper-tradable symbols, configured static paper-tradable symbols, and held static symbols. Count-only lane noise, timestamps, cooldown countdown changes, and repeated "no proposals/orders" state do not resend Telegram messages when the symbol sets and material state are unchanged. Material symbol-set changes, provider status changes, errors/warnings, catch-up completion, market phase changes, and user-requested status still send.
...
- **Provider material-change rules**: Market-closed provider snapshots compare endpoint status categories, active core cooldown endpoint names, optional cooldown endpoint names, plan-limited endpoint names, symbol-level no-data summaries, and provider error categories. Cooldown countdown timestamps and updated-at timestamps are ignored. Intraday/live endpoints are marked not needed while market closed instead of making the weekend status look globally unavailable. EODHD `eod_bars` 404/422 responses for individual symbols are classified as symbol-level no-data/not-found and do not mark the whole EOD endpoint as plan-limited or in core cooldown; `fundamentals` 403 remains plan-limited and non-gating.

- **Research-only vs trading preflight**: Core preflight keeps local safety basics. Research-only Dynamic Universe preflight requires internet, a writable DB, and the provider key when provider calls are needed; AC power is warning-only by default and broker/order functions are not required. If research completes on battery while internet and DB checks pass, research-status notification/suppression is still evaluated. Trading preflight remains strict and separately requires AC power when configured, internet, broker reachability, market-open when configured, Telegram/AI secrets, and all downstream RiskEngine/final-validation/broker safety controls before any proposal or order path can proceed.
- **Late power/network recovery**: launchd does not replay every missed interval after sleep, no internet, or battery operation. Dynamic Universe records missed schedule state and performs one bounded catch-up per schedule type when prerequisites recover. If that catch-up completes while Alpaca says the US market is open, Telegram wording must say `intraday catch-up refresh` and paper-only guarded trading rules, not `pre-market` or `blocked until market open`. If the market is still closed, catch-up wording remains explicitly trading-blocked.
