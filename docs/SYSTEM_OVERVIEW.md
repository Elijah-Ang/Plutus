...
- Crypto data prefers read-only Alpaca crypto market data when available. If the SDK/provider path is unavailable, crypto records missing/provider-unavailable evidence and creates no proposals. EODHD is research/context only and must not be used as a final trading price, tradability, position, open-order, execution, or approval-time revalidation source.
