...
| `ai_review.py` | Redacts proposal, uses Responses API, validates structured JSON, deterministic fallback | GPT has no broker object and cannot execute. Fallback is safe. The reviewer now honors the production `ai_max_calls_per_run` key, with the historical flat key retained only for isolated compatibility fixtures |
